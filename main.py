...
from utils.util import get_logger, epsilon
...
class Runner(object): 
    ...
    def train_model(self, train_loader, model, loss_fn, optimizer, desc='',epoch = None, eps_f = None):
        running_acc = 0.0
        running_loss = 0.0
        model.train()
        ...
        for batch_idx, batch in enumerate(t):
            ...
            scores, caps_sorted, decode_lengths, alphas, sort_ind = model(
                images, captions, lengths,eps_f.get_epsilon(epoch, batch_idx))
    ...
    def train(self, config_file, **kwargs):
        ...
        eps_mode = args["train_args"]["decay_mode"]
        num_batch = len(dataloaders["train"])
        eps_f = epsilon(num_epochs, num_batch, eps_mode)
        for epoch in range(num_epochs):
            train_loss = self.train_model(desc=f'Epoch {epoch + 1}/{num_epochs}',
                                          ...
                                          epoch = epoch,
                                          eps_f = eps_f)
            print(eps_f.get_epsilon(epoch,0))
            with torch.no_grad():
