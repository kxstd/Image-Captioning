...
class DecoderWithAttention(nn.Module):
    ...
    def forward(self, encoder_out, encoded_captions, caption_lengths, epsilon):
        """
        Forward propagation.
        ...
        for t in range(max(decode_lengths)):
            ...
            if random.random() < epsilon or t==0:
                ...
                sampled_embeddings = self.embedding(predicion_prev)
                h, c = self.decode_step(
                    torch.cat([sampled_embeddings[:batch_size_t,0], attention_weighted_encoding], dim=1),
                    (h[:batch_size_t], c[:batch_size_t])) 
            ...
            predicion_prev = preds.argmax(1).unsqueeze(1)
...
class Captioner(nn.Module):
    ...
    def forward(self, images, encoded_captions, caption_lengths, epsilon = None):
        """
        ...
        decoder_out = self.decoder(encoder_out, encoded_captions,
                                   caption_lengths.unsqueeze(1), epsilon = epsilon)
        return decoder_out
